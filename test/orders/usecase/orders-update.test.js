...
describe('update(:fileHandler, :orderId, :orderData)', () => {
  describe('When everything fine', () => {
    ...
    it('returns orders', async () => {
      const orderId = 1
      const orderData = {id: orderId, productsList: [{product: {id: 1, name: 'banana'}, quantity: 100}]}
      const orders = [{id: orderId, productsList: [{product: {id: 1, name: 'banana'}, quantity: 100}], status: 'pending'}]
      const fileHandlers = {
        orders: {
          read: jest.fn(() => orders),
          write: jest.fn()
      ...
      const createBill = jest.fn()
      const {update} = updateModule(fileHandlers.orders, updateTotalsList, createBill)
      const response = await update(orderId, orderData)
