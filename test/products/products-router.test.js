...
describe("Products", () => {
  beforeEach(async () => {
    ...
    await addProduct({ name: "banana", price: 2, weight: 0.2 });
    await addProduct({ name: "orange", price: 1.5, weight: 0.3 });
    await addProduct({ name: "vanilla", price: 10, weight: 0.01 });
  });
  describe("GET /products", () => {
    ...
    describe("when sort is called properly", () => {
      it("returns 200", done => {
        request(
          { url: `http://localhost:${PORT}/products?sort=weight` },
          (err, res) => {
            if (err) throw err;
            expect(res.statusCode).toBe(200);
            done();
      ...
      it("returns products sorted", done => {
        request(
          ...
          (err, res) => {
            if (err) throw err;
            const sortedByWeightProductsList = [
              { id: 2, name: "vanilla", price: 10, weight: 0.01 }
            ];
            expect(JSON.parse(res.body)[0]).toEqual(
              sortedByWeightProductsList[0]
            );
            done();
          }
        );
    ...
    describe("when sort is called with an invalid parameter", () => {
      it("returns 400", done => {
        ...
        request(
          { url: `http://localhost:${PORT}/products?sort=` + invalidParameter },
          (err, res) => {
            if (err) throw err;
        ...
        request(
          {
            url: `http://localhost:${PORT}/products`,
            method: "POST",
            json: { id: 3, name: "kiwi", price: 3, weight: 0.2 }
          },
          (err, res) => {
            if (err) console.log(err);
            expect(res.body.slice(-1)[0]).toEqual(json);
            done();
          }
        );
      });
