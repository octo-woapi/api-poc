function updateTotals(getProductById) {
  return order => {
    ...
    order.productsList.map(product =>
      Object.assign(product.product, getProductById(product.product.id))
    );
...
function computeWeight(productsInfo) {
  const result = productsInfo.reduce((a, b) => a + b.product.weight * b.quantity, 0);
  return result;
}

function computePrice(productsInfo) {
  return productsInfo.reduce((a, b) => a + b.product.price * b.quantity, 0);
}

function calculateShippingAmount(weight) {
  const WEIGHT_STEP = 10;
  const STEP_PRICE = 25;
  return Math.floor(weight / WEIGHT_STEP) * STEP_PRICE;
}
